...
def ours2sigma(cosmicf, ourf, sigmaf):
    """
    input: 96 count file, tab separated, 1st column id
    output: 96 count file, comma separated, last column id
    """
    ...
    # add tumor column
    id_col = pd.read_csv(ourf, sep='\t').values[:,0]
    mutmat = pd.read_csv(ourf, sep='\t').values[:,1:]
    sigma_df = pd.DataFrame(mutmat, columns=new_name)
    sigma_df['tumor'] = id_col
...
if __name__ == "__main__":
    msk_dir = "/Users/yuexichen/Desktop/lrgr_file/mskfiles/jan_downsize/natmed2019"
    ...
    ourf=join(msk_dir, "filter-staaf2019_BRCA-WGS_counts.tsv")
    sigmaf=join(msk_dir, "sigma-filter-staaf2019_BRCA-WGS_counts.tsv")

    ours2sigma(cosmicf, ourf, sigmaf)
