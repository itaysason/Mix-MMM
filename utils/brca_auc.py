...
if __name__ == "__main__":
    ...
    doc_f = join(msk_dir, "/Users/yuexichen/Downloads/lrgr_file/mskfiles/part1-status-new-OV-ds5-OV-ds5-survival-analysis-assignments-0.tsv")
    ...
    ds_id = pd.read_csv(join(msk_dir, "mix_downsize/ov-downsize5_sample_id.txt"),sep='\n', header=None).values.tolist()
    mut_dict = {}
    ...
    part_ls = [1,2]
    index_all = np.load("/Users/yuexichen/Downloads/lrgr_file/mskfiles/mix_downsize/indices/OV-ds5-indices.npy")
    
    for p in part_ls:
        if method == "Mix":
            expo_f = join(msk_dir, "mix_downsize/models/OV-ds5-part%d-exposure.npy"%p)
            expo_np = np.load(expo_f)
            all_y_score = [item[1]/sum(item) for item in expo_np]
        elif method == "SigMA":
            expo_f = join(msk_dir,"mix_downsize/sigma_downsize5_sig3_part%d.npy"%p)
            expo_np = np.load(expo_f)
            all_y_score = expo_np
