...
def correlate_sigs_hrd_nature2019(normalized=True):
    # Find the best model
    directory = os.path.join(ROOT_DIR, 'experiments/trained_models/ICGC-BRCA/refit')
    model = get_best_model(directory, return_model=True)
    num_sigs = model.num_topics
    ...
    labels = labels[:, 0]
    print(pearsonr(data.sum(1), labels))

    exposures = model.weighted_exposures(data)
    if not normalized:
        exposures *= data.sum(1, keepdims=True)

    print('\nMIX correlations')
    for i in range(num_sigs):
        curr_sigs = exposures[:, i]
        pearson_corr, pearson_p_value = pearsonr(curr_sigs, labels)
        spearman_corr, spearman_p_value = spearmanr(curr_sigs, labels)
        print('Sig{} - pearson: correlation {:.3f}, p_value {:.3f} | spearman: correlation {:.3f}, p_value {:.3f}'.
              format(sigs[i] + 1, pearson_corr, pearson_p_value, spearman_corr, spearman_p_value))
    cca = CCA(n_components=1)
    U_c, V_c = cca.fit_transform(exposures, labels[:, np.newaxis])
    print(np.corrcoef(U_c.T, V_c.T)[0, 1])
    ...
    for i in range(num_sigs):
        ...
        pearson_corr, pearson_p_value = pearsonr(curr_sigs, labels)
        spearman_corr, spearman_p_value = spearmanr(curr_sigs, labels)
        print('Sig{} (similarity {}) - pearson: correlation {:.3f}, p_value {:.3f} | spearman: correlation {:.3f}, p_value {:.3f}'.
              format(sigs[i], corrs[i], pearson_corr, pearson_p_value, spearman_corr, spearman_p_value))
    cca = CCA(n_components=1)
    U_c, V_c = cca.fit_transform(exposures, labels[:, np.newaxis])
    print(np.corrcoef(U_c.T, V_c.T)[0, 1])
    ...
    for i in range(exposures.shape[1]):
        ...
        pearson_corr, pearson_p_value = pearsonr(curr_sigs, labels)
        spearman_corr, spearman_p_value = spearmanr(curr_sigs, labels)
        print('{} - pearson: correlation {:.3f}, p_value {:.3f} | spearman: correlation {:.3f}, p_value {:.3f}'.
              format(col_names[i], pearson_corr, pearson_p_value, spearman_corr, spearman_p_value))
    cca = CCA(n_components=1)
    U_c, V_c = cca.fit_transform(exposures, labels[:, np.newaxis])
    print(np.corrcoef(U_c.T, V_c.T)[0, 1])

    exposures = sigma_output_to_exposures('data/nature2019/SigMA_output_full.tsv')
    sigs = np.arange(1, 31)
    ...
    for i in range(exposures.shape[1]):
        ...
        spearman_corr, spearman_p_value = spearmanr(curr_sigs, labels)
        print('{} - pearson: correlation {:.3f}, p_value {:.3f} | spearman: correlation {:.3f}, p_value {:.3f}'.
              format(sigs[i], pearson_corr, pearson_p_value, spearman_corr, spearman_p_value))
    cca = CCA(n_components=1)
    U_c, V_c = cca.fit_transform(exposures, labels[:, np.newaxis])
    print(np.corrcoef(U_c.T, V_c.T)[0, 1])
...
def analyze_msk2018():
    ...
    models = ['NUM_MUT', 'data', 'MIX_MSK-ALL_refit', 'MIX_MSK-ALL_refit_normalized', 'MIX_MSK-ALL_denovo',
              'MIX_MSK-ALL_denovo_normalized', 'NNLS_MSK-ALL', 'NNLS', 'SigMA',
              'MIX_MSK-ALL_denovo_cluster-prob_normalized', 'MIX_MSK-ALL_refit_cluster-prob_normalized']

    models = ['NNLS_MSK-ALL', 'MIX_MSK-ALL_refit_normalized', 'MIX_MSK-ALL_denovo_normalized']
    for model_name in models:
        print(model_name)
        # Get exposures
        feature_names = []
        if model_name == 'SigMA':
            exposures = sigma_output_to_exposures(
                'data/immunotherapy_response/panel_nsclc_pd1_msk_2018/out_sigma_msk2018_LUAD.tsv')
            sigs = np.where(exposures.sum(0) > 0)[0]
            exposures = exposures[:, sigs]
        elif model_name == 'MIX_MSK-LUAD_refit_normalized':
            directory = os.path.join(ROOT_DIR, 'experiments/trained_models/MSK-LUAD/refit')
            model = get_best_model(directory, return_model=True)
            exposures = model.weighted_exposures(data)
            _, sigs = get_data('MSK-ALL')
        elif model_name == 'MIX_MSK-LUAD_refit':
            directory = os.path.join(ROOT_DIR, 'experiments/trained_models/MSK-LUAD/refit')
            model = get_best_model(directory, return_model=True)
            exposures = model.weighted_exposures(data)
            exposures *= data.sum(1, keepdims=True)
            _, sigs = get_data('MSK-ALL')
        elif model_name == 'MIX_MSK-ALL_denovo_cluster-prob_normalized':
            directory = os.path.join(ROOT_DIR, 'experiments/trained_models/MSK-ALL/denovo')
            model = get_best_model(directory, return_model=True)
            exposures = model.soft_cluster(data)
            exposures = exposures[:, [0, 4]]
            exposures = exposures.sum(1, keepdims=True)
            feature_names = ['cluster 0+4']
            # feature_names = ['cluster {}'.format(i + 1) for i in [0, 4]]
        elif model_name == 'sig_correlation':
            cosmic = get_cosmic_signatures()[sigs]
            exposures = np.zeros((len(data), len(sigs)))
            for i in range(len(data)):
                for j in range(len(sigs)):
                    exposures[i, j] = cosine_similarity(data[i], cosmic[j])[0, 1]

        else:
            exposures, feature_names = get_features(model_name, data, sigs)

        if len(feature_names) == 0:
            feature_names = ['{}'.format(sigs[i]) for i in range(len(sigs))]

        # Score features individually
        individual_score_dict = score_per_feature(exposures, labels)
        individual_score_dict['feature_name'] = feature_names
        tmp = pd.DataFrame(individual_score_dict)
        tmp.to_csv('{}.tsv'.format(model_name), sep='\t', index=False)
        # print(pd.DataFrame(individual_score_dict).values[:, :-1].astype('float').round(3))

        # Score the whole model
        total_score_dict = score_all_features(exposures, labels)
        print(total_score_dict['auc'].round(3))
        print(total_score_dict['auc_loo'].round(3))
        # print('\n')


def predict_immunotherapy_allen2018():
    # Find the best model
    data, sigs = get_data('allen2018-Melanoma')
    labels, _ = get_data('allen2018-Melanoma-labels')

    models = ['NUM_MUT', 'MIX_MSK-ALL_refit_normalized', 'MIX_MSK-ALL_refit', 'MIX_MSK-ALL_denovo_normalized',
              'MIX_MSK-ALL_denovo', 'NNLS', 'NNLS_MSK-ALL', 'MIX_MSK-ALL_cluster-prob_normalized',
              'MIX_MSK-ALL_denovo_cluster-prob_normalized']
    for model_name in models:
        print(model_name)
        # Get exposures
        exposures, feature_names = get_features(model_name, data, sigs)
...
def analyze_hrd():
    ...
    labels = labels[:, 0]
    good_indices = labels != 0
    labels = labels[labels != 0]
    labels[labels == -1] += 1
    labels = labels.astype('int')

    models = ['MIX_BRCA_refit_normalized', 'MIX_BRCA_refit']
    for model_name in ['SigMA-sig3', 'MIX_BRCA_refit_normalized', 'NNLS', 'SigMA']:
        print(model_name)
        # Get exposures
        feature_names = []
        if model_name == 'SigMA':
            ...
            sigs = np.where(exposures.sum(0) > 0)[0]
            exposures = exposures[:, sigs]
        elif model_name == 'SigMA-clusters':
            exposures = sigma_output_to_clusters('data/nature2019/SigMA_output_full.tsv')
            feature_names.extend(['Signature_17', 'Signature_3_hc', 'Signature_3_lc', 'Signature_8', 'Signature_APOBEC',
                                  'Signature_clock'])
        elif model_name == 'SigMA-sig3':
            all_df = pd.read_csv('data/nature2019/SigMA_output_full.tsv', sep='\t')
            exposures = all_df.Signature_3_mva.values[:, np.newaxis]
            feature_names.extend(['Signature_3mva'])
        elif model_name == 'MIX_BRCA_refit_normalized':
            directory = os.path.join(ROOT_DIR, 'experiments/trained_models/BRCA-panel/refit')
            model = get_best_model(directory, return_model=True)
            exposures = model.weighted_exposures(data)
            _, sigs = get_data('ICGC-BRCA')
        elif model_name == 'MIX_BRCA_refit':
            directory = os.path.join(ROOT_DIR, 'experiments/trained_models/BRCA-panel/refit')
            model = get_best_model(directory, return_model=True)
            exposures = model.weighted_exposures(data)
            exposures *= data.sum(1, keepdims=True)
        ...
        exposures = exposures[good_indices]
        ...
        total_score_dict = score_all_features(exposures, labels)
        # print(total_score_dict['auc'].round(3))
        # print(total_score_dict['auc'].round(3), total_score_dict['maximized_auc_estimated'].round(3),
        #       total_score_dict['maximized_auc_exact'].round(3))
        print('\n')


def predict_hrd():
    # for MODEL in ['MIX', 'SigMA_panel', 'NNLS', 'MIX-clusters']:
    for MODEL in ['MIX']:
        exposure_types = ['cluster']
        if MODEL == 'MIX':
            # exposure_types = ['cluster', 'weighted']
            exposure_types = ['weighted']
        if MODEL == 'SigMA_panel':
            exposure_types = ['cluster', 'exposures']

        for exposure_type in exposure_types:
            normaliziation_types = [1, 0]
            if MODEL == 'SigMA_panel' or MODEL == 'NNLS':
                normaliziation_types = [0]
            for NORMALIZED_EXPOSURES in normaliziation_types:
                print('{}, {}, {}'.format(MODEL, exposure_type, 'normalized' if NORMALIZED_EXPOSURES else 'not normalized'))
                ...
                if MODEL == 'MIX':
                    ...
                    if exposure_type == 'weighted':
                        train_data = model.weighted_exposures(train_mutations)
                    elif exposure_type == 'cluster':
                        cluster_probs = model.soft_cluster(train_mutations)
                        clusters = np.argmax(cluster_probs, 1)
                        train_data = model.pi[clusters]
                    elif exposure_type == 'prediction':
                ...
                elif MODEL == 'MIX-clusters':
                    directory = os.path.join(ROOT_DIR, 'experiments/trained_models/BRCA-panel/refit')
                    train_mutations, _ = get_data('BRCA-panel')

                    model = get_best_model(directory, return_model=True)
                    train_data = model.soft_cluster(train_mutations)
                elif MODEL == 'SigMA_panel':
                    path = 'data/out-sigma-filter-BRCA-WGS_counts.tsv'
                    if exposure_type == 'exposures':
                        train_data = sigma_output_to_exposures(path)
                    elif exposure_type == 'cluster':
                        train_data = sigma_output_to_clusters(path)
                    else:
                        raise ValueError('No such model {}'.format(MODEL))
                elif MODEL == 'NNLS':
                    train_mutations, signatures = get_data('BRCA-panel')
                    train_data = stack_nnls(train_mutations, get_cosmic_signatures()[signatures])
                else:
                    raise ValueError('No such model {}'.format(MODEL))

                # train_mutations, _ = get_data('BRCA-panel')
                # train_data = np.column_stack((train_data, train_mutations.sum(1, keepdims=True)))
                ...
                classifier = LogisticRegression()
                classifier.fit(train_data, train_labels)
                # print(classifier.coef_[0].round(3))
                ...
                if MODEL == 'MIX':
                    ...
                    if exposure_type == 'weighted':
                        ...
                        cluster_probs = model.soft_cluster(test_mutations)
                        clusters = np.argmax(cluster_probs, 1)
                    ...
                    test_mutations, _ = get_data('nature2019-panel')
                    test_data = model.soft_cluster(test_mutations)
                elif MODEL == 'SigMA_panel':
                    path = 'data/nature2019/SigMA_output_full.tsv'
                    if exposure_type == 'exposures':
                        test_data = sigma_output_to_exposures(path)
                    elif exposure_type == 'cluster':
                        test_data = sigma_output_to_clusters(path)
                    else:
                        raise ValueError('No such model {}'.format(MODEL))
                elif MODEL == 'NNLS':
                ...
                # test_mutations, _ = get_data('nature2019-panel')
                # test_data = np.column_stack((test_data, test_mutations.sum(1, keepdims=True)))
                ...
                prediction_probabilities = classifier.predict_proba(test_data)[:, 1]
...
# predict_hrd()
# analyze_msk2018()
# predict_immunotherapy_allen2018()
# plot_cluster_AMI(range(4, 7))
# plot_sig_correlations('MSK-ALL', range(12, 13), plot_title=False, save_plot=True)

# data = get_data('survival_data')
#
# directory = os.path.join(ROOT_DIR, 'experiments/trained_models/BRCA-panel/refit')
# train_mutations, _ = get_data('BRCA-panel')
#
...
#
#
# a, signatures = get_data('Gali')
# e = get_cosmic_signatures()[signatures]
# mmm = MMM(len(signatures), init_params={'e': e})
# mmm.refit(a)
# exposures = mmm.pi
# df_dict = {'sample': pd.read_csv('data/Gali/processed_data.tsv', sep='\t').values[:, 0]}
# for i in range(len(signatures)):
#     df_dict['signature {} exposure'.format(signatures[i] + 1)] = exposures[:, i]
# df = pd.DataFrame(df_dict)
# df.to_csv('output_mmm.tsv', sep='\t', index=False)
#
#
# from src.models.biterm import biterm
#
#
# a, signatures = get_data('Gali')
# e = get_cosmic_signatures()[signatures]
# mmm = biterm(len(signatures), init_params={'e': e})
# mmm.refit(a)
# exposures = mmm.pi
# df_dict = {'sample': pd.read_csv('data/Gali/processed_data.tsv', sep='\t').values[:, 0]}
# for i in range(len(signatures)):
#     df_dict['signature {} exposure'.format(signatures[i] + 1)] = exposures[:, i]
# df = pd.DataFrame(df_dict)
# df.to_csv('output_BTM.tsv', sep='\t', index=False)
